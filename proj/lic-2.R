...
chisq.test(train$Insurance_History_1,train$InsuredInfo_2)
chisq.test(train$Ins_Age,train$InsuredInfo_1)
...
Data_1 <- All_Data[,names(All_Data) %in% c("Product_Info_4",	"Ins_Age",
          "Ht",	"Wt",	"BMI",	"Employment_Info_1",	"Employment_Info_4",
          ...
          "Medical_History_32",paste("Medical_Keyword_",1:48,sep=""))]

str(Data_1)

Data_2 <- All_Data[,!(names(All_Data) %in% c("Product_Info_4",	"Ins_Age",	"Ht",
          "Wt",	"BMI",	"Employment_Info_1",	"Employment_Info_4",
          "Employment_Info_6",	"Insurance_History_5",	"Family_Hist_2",
          "Family_Hist_3",	"Family_Hist_4",	"Family_Hist_5",
          "Medical_History_1",	"Medical_History_15",	"Medical_History_24",
          "Medical_History_32",paste("Medical_Keyword_",1:48,sep="")))]
...
#Make a new variable which is equivalent to the quintile groups of Wt, we can
#use the group_into_buckets function we defined above
...
#We should investigate what this impact is and ensure we are not overfitting. 
#The train_30 data would help for this, for example we could look at some actual 
#versus predicted plots across these variables on the train_30 etc.
...
dim(Prediction_Object)
tail(Prediction_Object)
